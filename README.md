...
If you've used other configuration management tools for this kind of task, hopefully this project can show you a much improved alternative. Because of Nix, we get a provisioning that's deep, precise, idempotent, and highly repeatable, much moreso than with tools like Ansible. Additionally, almost the entire system is built by a continuous integration job, and published to a public cache for convenient redistribution. So we're getting benefits similar to those of Docker, but without the restriction to containers.
