...
[This project's continuous integration (using GitHub Actions)](https://github.com/shajra/shajra-provisioning/actions) caches built packages at [Cachix](https://cachix.org/), a service for caching pre-built Nix packages. If you don't want to wait for a full local build when first using this project, setting up Nix to pull from Cachix is recommended.
...
```
    ...
    * pkgs.aspellDicts-en (aspell-dict-en)
      Aspell dictionary for English
    
    * pkgs.aspellDicts-en-computers (aspell-dict-en-computers)
...
The following result is one returned by our prior execution of `nix search --no-cache --file .`:
    ...
    * pkgs.python38Packages-grip (python3.8-grip)
      Preview GitHub Markdown files like Readme locally before committing them

We can see that a package named "python3.8-grip" can be accessed with the `pkgs.python38Packages-grip` attribute path in the Nix expression in the project root's `default.nix`. Not shown in the search results above, this package happens to provide the executable `grip`.

We can build this package with `nix build` from the project root:

```shell
nix build --file . pkgs.python38Packages-grip
```
...
```
    ...
    /nix/store/vfq4p8qzq1qprn3psmzh07450hizb2jw-python3.8-grip-4.5.2
...
For example, to get the help message for the `grip` executable provided by the `python3.8-grip` package selected by the `pkgs.python38Packages-grip` attribute path from `.`, we can call the following:

```shell
nix run \
    --file . \
    pkgs.python38Packages-grip \
    --command grip --help
```
...
To install the `grip` executable, which is accessed by the `pkgs.python38Packages-grip` in our top-level `default.nix` file, we'd run the following:

```shell
nix-env --install --file . --attr pkgs.python38Packages-grip 2>&1
```
