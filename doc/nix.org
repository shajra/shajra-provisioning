...
Note that this document only covers the Nix package manager, an not [[nixos][NixOS]] (a
full Linux operating system built on top of Nix) or [[nix-darwin][Nix-Darwin]] (a project that
gives the benefits of NixOS for MacOS).
...
expressions could evaluate to the exact same derivation, but use different
attribute paths. This is why we uninstall packages by their package name.
