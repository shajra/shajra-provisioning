...
** Building Nix expressions
   ...
   From our execution of =nix search= above we can see that a package named
   {{{package-name}}} can be accessed with the {{{package-attr}}} attribute
   path in the Nix expression in the project root's =default.nix=.
...
** Installing and uninstalling programs
   ...
   #+BEGIN_SRC shell :session env :dir .. :results output :exports both :noweb yes :post crop(text=*this*, first-n=0, last-n=3)
