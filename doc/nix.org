...
** Variables
   ...
   #+MACRO: package-attr "binutils-unwrapped"
   #+MACRO: package-name "binutils"
   #+MACRO: package-exe =addr2line=
...
** Running commands
   ...
   =nux run= also supports an =--ignore-environment= flag that restricts =PATH=
...
** Installing and uninstalling programs
   ...
   #+BEGIN_SRC shell :eval no :noweb yes
     nix-env --install --file . --attr <<package-attr>>
   #+END_SRC

   #+BEGIN_SRC shell :dir .. :results silent :exports none :noweb yes
     mkdir nix-profile
     nix-env --profile nix-profile/doc --install --file . --attr <<package-attr>>
   #+END_SRC
   ...
   #+BEGIN_SRC shell :dir .. :results silent :exports none :noweb yes
     mkdir nix-profile
     nix-env --profile nix-profile/doc --uninstall <<package-name>>
   #+END_SRC
...
* Next Steps
  ...
  be used for setting up development environments. And we didn't cover [[nixpkgs][Nixpkgs]],
  a gigantic repository of community-curated Nix expressions.
