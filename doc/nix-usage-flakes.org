#+title: Nix End-user Guide with Flakes
...
each of which contains an expression written in the [[nix-manual-language][Nix expression language]] used
...
=nix build= has a =--no-link= option in case you want to build packages without
...
The command to run within the shell is specified after the =--command= option.
...
can drop the =--command= option and following arguments.
...
concise syntax than =nix shell= with a =--command= option.
...
We can see some of the metadata of this package with the =--json= option of =nix
search=:
...
options/arguments can feel extraneous. It would be nice to have the programs on
...
to another profile at any writable location with the =--profile= option.
...
All the commands we've covered have more options and options. See the
