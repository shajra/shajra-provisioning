...
Though the latest version of Nix is Nix 2.34.7, we'll be installing the version that the last release of NixOS (25.11) uses, specifically Nix 2.31.5. As discussed in the included [introduction to Nix](nix-introduction.md), this version is considered stable by the Nix community.
...
```bash
sh <(curl -L https://releases.nixos.org/nix/nix-2.31.5/install) --daemon
```
