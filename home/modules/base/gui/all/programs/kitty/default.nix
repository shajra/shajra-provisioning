...
let
  ...
  hintsBg = pkgs.lib.colors.colorName config.theme.colors.semantic.highlight config.theme.colors.terminal;

  hintsFg = pkgs.lib.colors.colorName (config.theme.colors.nominal.foregroundFor hintsBg) config.theme.colors.terminal;

  hintsOpen =
    type:
    "kitten hints --type=${type}"
